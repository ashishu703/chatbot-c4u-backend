...
module.exports = class InstagramAccountRepository {
  ...
  static async findByAccountId(accountId) {
    const pages = await query('SELECT * FROM instagram_accounts WHERE account_id = $1', [accountId]);
  ...
  static async deleteByAccountId(accountId) {
    return query('DELETE FROM instagram_accounts WHERE account_id = $1', [accountId]);
  }

  static async findManyByUserId(userId) {
    const accounts = await query('SELECT * FROM instagram_accounts WHERE uid = $1', [userId]);
    return accounts;
  }

  static async findOneByUserIdAndAccountId(userId, accountId) {
    const account = await query('SELECT * FROM instagram_accounts WHERE uid = $1 AND account_id = $2', [userId, accountId]);
    return account.length > 0 ? account[0] : null;
  }

  static async countByUserId(userId) {
    const result = await query('SELECT COUNT(*) AS count FROM instagram_accounts WHERE uid = $1', [userId]);
