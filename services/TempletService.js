...
class TempletService {
  ...
  async prepareTemplate(template, example, dynamicMedia) {
    try {
      const checkBody = template?.components?.filter((i) => i.type === "BODY");
      const getHeader = template?.components?.filter(
        (i) => i.type === "HEADER"
      );
      ...
      if (headerFormat === "IMAGE" && getHeader.length > 0) {
        const media = await this.metaTempleteMediaRepository.findFirst({
